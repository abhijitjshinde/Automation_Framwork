...
class SeleniumDriver():
    ...
    def screenShot(self, resultMessage):
        ...
        """
        print("Inside SS ######################################################")
        self.log.info("Inside SS ######################################################")
        current_date = str(date.today())
        self.log.info("Inside S1 ######################################################")

        loggerName = inspect.stack()[3][3]
        fileName = str(loggerName) +"_"+str(time.strftime("%d-%m-%Y %H_%M_%S")) + ".png"
        screenshotDirectory = "..\\screenshots\\"+current_date+"\\"
        self.log.info("Inside S2 ######################################################")
        relativeFileName = screenshotDirectory + fileName
        currentDirectory = os.path.dirname(__file__)
        destinationFile = os.path.join(currentDirectory, relativeFileName)
        destinationDirectory = os.path.join(currentDirectory, screenshotDirectory)
        self.log.info("Inside S3 ######################################################")
        ...
        try:
            ...
            if not os.path.exists(destinationDirectory):
                ...
                print("Inside Mkdir ##################################################")
                self.log.info("Inside Mkdir ##################################################")
            self.driver.save_screenshot(destinationFile)
            print("SS Taken #####################################################")
            self.log.info("SS Taken #####################################################")
    ...
    def sendKeysbywords(self, data, locator="", locatorType="id", element=None):
        """
        Send keys to an element -> MODIFIED
        Either provide element or a combination of locator and locatorType
        """
    ...
    def getText(self, locator="", locatorType="id", element=None, info=""):
        ...
        try:
            ...
            if len(text) != 0:
                ...
                text = text.strip()
                self.log.info("The text is :: '" + text + "' striped ..........")
        except:
            self.log.error("Failed to get text on element " + info)
            print_stack()
            text = None
        self.log.info("The text is :: going to return '" + str(text) + "'.........")
        return text
    ...
    def selectDropdwon(self, SelectText, locator="", locatorType="id", element=None):
        """
        select the dropdown through text,locator
        """
        try:
            if locator:  # This means if locator is not empty
                element = self.getElement(locator, locatorType)
            ...
            self.log.info("Select the text: "+SelectText+" with locator: " + locator +
                          " locatorType: " + locatorType)
        except:
            self.log.info("Cannot Select the text: "+SelectText+" with locator: " + locator +
                          " locatorType: " + locatorType)
            print_stack()
